...
export default function ExternalApiSettings() {
  ...
  const example = `curl "${endpoint}/tasks?query=invoice" \\
  ...
  return <section className="settings-card external-api-card">
    ...
    : <>
      ...
      <div className="api-endpoint-panel">
        <span>OpenAPI</span><code>{endpoint}/openapi.json</code><button className="ghost-button compact" onClick={()=>void copy(`${endpoint}/openapi.json`)}><Copy size={13}/> Copy</button>
      </div>
      ...
      <details className="api-example"><summary>Example API call</summary><pre><code>{example}</code></pre><p>REST endpoints are available for tasks, completion/reopen/restore, and projects including rename/delete. The legacy <code>POST /api/taskmap</code> action endpoint remains supported for existing integrations.</p></details>
    </>}
